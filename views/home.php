      ...
      <p>
        <?php if(count($child['photos']) > 0) { ?>
          ...
          <?php foreach($child['photos'] as $num => $photo) { ?>
            <li><img src="<?php echo !empty($photo['p_thumbPath']) ? Photo::generateUrl($photo['p_basePath'], 100, 100, array(Photo::square)) : '/img/processing.png'; ?>" class="frame-100" title="<?php echo quoteEncode($child['c_name']) . ' at ' . displayAge($child['c_birthdate'], $photo['p_dateTaken']); ?> old."/></li>
            <?php if($num >= 11) break; ?>
          <?php } ?>
          </ul>
          <br>

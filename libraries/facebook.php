...
class Facebook
{
  ...
  public function getAuthorizeUrl($redirectUrl=null, $params = null) {
    if(empty($redirectUrl))
      $redirectUrl = $this->getCurrentUrl();
    $redirectUrl = urlencode($redirectUrl);
    $url = self::$DOMAIN_MAP['graph'] . "oauth/authorize?client_id={$this->getAppId()}&redirect_uri={$redirectUrl}";
    if(!empty($params))
      $url .= '&' . http_build_query($params);
    return $url;
  }
 
  public function fetchAccessToken($code, $redirectUrl, $params=null) {
    $redirectUrl = urlencode($redirectUrl);
    $url = self::$DOMAIN_MAP['graph'] . "oauth/access_token?client_id={$this->getAppId()}&redirect_uri={$redirectUrl}&"
